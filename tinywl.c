...
static void set_ssd_enabled(struct tinywl_toplevel *toplevel, bool enabled) {
	wlr_scene_node_set_enabled(&toplevel->titlebar->node, enabled);
	wlr_scene_node_set_enabled(&toplevel->border_left->node, enabled);
	wlr_scene_node_set_enabled(&toplevel->border_right->node, enabled);
	wlr_scene_node_set_enabled(&toplevel->border_bottom->node, enabled);
	wlr_scene_node_set_enabled(&toplevel->button_close->node, enabled);
	wlr_scene_node_set_enabled(&toplevel->button_minimize->node, enabled);
	wlr_scene_node_set_enabled(&toplevel->button_maximize->node, enabled);
}

static void focus_toplevel(struct tinywl_toplevel *toplevel, struct wlr_surface *surface) {
	/* Note: this function only deals with keyboard focus. */
	if (toplevel == NULL || toplevel->minimized || toplevel->animation != NULL) {
		return;
	}
...
static bool start_genie_animation(struct tinywl_toplevel *toplevel,
	...
	struct wlr_scene_rect *ssd_rects[] = {
		...
		toplevel->button_minimize,
		toplevel->button_maximize,
	};
	struct tinywl_genie_options options = {
		...
		.source = &toplevel->content_tree->node,
		.source_x = tx,
		.source_y = ty,
		.rects = ssd_rects,
		.rect_count = toplevel->client_side_decorated ? 0 :
			sizeof(ssd_rects) / sizeof(ssd_rects[0]),
		.window = {
			.x = tx - (toplevel->client_side_decorated ? 0 : SSD_BORDER_WIDTH),
			.y = ty - (toplevel->client_side_decorated ? 0 : SSD_TITLE_HEIGHT),
			.width = toplevel->content_width +
				(toplevel->client_side_decorated ? 0 : 2 * SSD_BORDER_WIDTH),
			.height = toplevel->content_height +
				(toplevel->client_side_decorated ? 0 :
					SSD_TITLE_HEIGHT + SSD_BORDER_WIDTH),
		},
		.target_x = output_box.x + output_box.width * 0.5,
		.target_y = output_box.y + output_box.height -
			DOCK_BOTTOM_MARGIN - DOCK_HEIGHT + 41,
...
static struct tinywl_toplevel *latest_minimized_toplevel(
		struct tinywl_server *server) {
...
static struct tinywl_toplevel *active_visible_toplevel(
		struct tinywl_server *server) {
...
static void set_toplevel_maximized(struct tinywl_toplevel *toplevel,
	...
	if (maximized) {
		toplevel->restore_x = toplevel->scene_tree->node.x;
		toplevel->restore_y = toplevel->scene_tree->node.y;
		toplevel->restore_width = toplevel->content_width;
		toplevel->restore_height = toplevel->content_height;
		int inset_x = toplevel->client_side_decorated ? 0 : SSD_BORDER_WIDTH;
		int inset_y = toplevel->client_side_decorated ? 0 :
			SSD_TITLE_HEIGHT + SSD_BORDER_WIDTH;
		wlr_scene_node_set_position(&toplevel->scene_tree->node,
			box.x + inset_x, box.y + inset_y);
		wlr_xdg_toplevel_set_size(toplevel->xdg_toplevel,
			box.width - (toplevel->client_side_decorated ? 0 : 2 * SSD_BORDER_WIDTH),
			box.height - inset_y -
				(toplevel->client_side_decorated ? 0 : SSD_BORDER_WIDTH));
	} else {
		wlr_scene_node_set_position(&toplevel->scene_tree->node,
...
static void server_cursor_button(struct wl_listener *listener, void *data) {
	...
	if (event->button == BTN_LEFT && surface != NULL) {
		...
		if (layer_surface != NULL && layer_surface->namespace != NULL &&
				...
				sx >= 24 && sx < 88 && sy >= 9 && sy < 73) {
			struct tinywl_toplevel *minimized = latest_minimized_toplevel(server);
			if (minimized != NULL && start_genie_animation(minimized,
					layer_surface->output, false)) {
				server->consume_dock_click = true;
				return;
			}
			struct tinywl_toplevel *visible = active_visible_toplevel(server);
			if (visible != NULL && start_genie_animation(visible,
					layer_surface->output, true)) {
				server->consume_dock_click = true;
				return;
			}
		}
	...
	double ry = server->cursor->y - ty;
		...
		if (rx >= 12 && rx < 12 + SSD_BUTTON_SIZE) {
			...
			if (rx >= 32 && rx < 32 + SSD_BUTTON_SIZE) {
...
static void set_decoration_mode(struct tinywl_toplevel *toplevel,
		enum wlr_xdg_toplevel_decoration_v1_mode mode) {
	bool client_side = mode ==
		WLR_XDG_TOPLEVEL_DECORATION_V1_MODE_CLIENT_SIDE;
	toplevel->client_side_decorated = client_side;
	set_ssd_enabled(toplevel, !client_side);
...
static void decoration_request_mode(struct wl_listener *listener, void *data) {
	...
	set_decoration_mode(toplevel, toplevel->decoration->requested_mode);
}
...
static void decoration_destroy(struct wl_listener *listener, void *data) {
	...
	toplevel->client_side_decorated = false;
	set_ssd_enabled(toplevel, true);
}

static void server_new_decoration(struct wl_listener *listener, void *data) {
	struct wlr_xdg_toplevel_decoration_v1 *decoration = data;
	struct wlr_scene_tree *content_tree = decoration->toplevel->base->data;
	if (content_tree == NULL || content_tree->node.parent == NULL) {
		wlr_xdg_toplevel_decoration_v1_set_mode(decoration,
			WLR_XDG_TOPLEVEL_DECORATION_V1_MODE_SERVER_SIDE);
		return;
	}
	struct tinywl_toplevel *toplevel = content_tree->node.parent->node.data;
	if (toplevel == NULL) {
		wlr_xdg_toplevel_decoration_v1_set_mode(decoration,
			WLR_XDG_TOPLEVEL_DECORATION_V1_MODE_SERVER_SIDE);
		return;
